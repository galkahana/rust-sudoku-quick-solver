...
use crate::{
    ...
    guesses_generation::{generate_group_of_number_guesses, generate_groups_of_cell_guesses},
};
...
impl SudokuGame {
    ...
    pub fn solve(
        &mut self,
        board: Board,
        guess_method: GuessMethod,
        backtracking_depth_limit: u32,
    ) -> (bool, Board) {
    ...
    fn search_solution(
        &mut self,
        constraints: &mut BoardConstraints,
        guess_method: GuessMethod,
        backtracking_depth_limit: u32,
    ) -> bool {
        ...
        if constraints.get_board().is_full() {
            ...
            if constraints.has_conflicts() {
                ...
                return self.search_solution_with_backtracking(
                    constraints,
                    guess_method,
                    backtracking_depth_limit,
                );
            }
        }
    }

    fn search_solution_with_backtracking(
        &mut self,
        constraints: &mut BoardConstraints,
        guess_method: GuessMethod,
        backtracking_depth_limit: u32,
        ...
        self.latest_solution_stats.max_reached_depth = cmp::max(
            self.latest_solution_stats.max_reached_depth,
            self.latest_solution_stats.depth_limit - backtracking_depth_limit,
        );
        ...
        let guesses = match guess_method {
            GuessMethod::GuessMethodCell => {
                generate_groups_of_cell_guesses(constraints.get_cell_constraints())
            }
            GuessMethod::GuessMethodNumber => {
                generate_group_of_number_guesses(constraints.get_cell_constraints())
            }
        };
        ...
        for guess_group in guesses {
            for guess in guess_group.get_guesses() {
                ...
                if self.search_solution(
                    &mut pending_constraints,
                    guess_method,
                    backtracking_depth_limit - 1,
                ) {

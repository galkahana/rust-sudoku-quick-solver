...
fn main() {
    ...
    let (result_status, result_board) =
        game.solve(board, sudoku_game::GuessMethod::GuessMethodNumber, 2);

    if !result_status {
        bar.finish_with_message(format!(
            "Could not solve puzzle. here's some stats {:?}",
            game.get_latest_solution_stats()
        ));
    } else {
